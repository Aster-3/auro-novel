...
const WriteReviewScreen = () => {
  ...
  const { mutateAsync, isPending } = useCommentMutation();
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      setVisibleCount(commentText.length);
    }, 180);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await mutateAsync({ novelId, content, isRecommend: recommend });
      ...
      useToastStore.getState().showToast({
        message: "Yorum yayınlanamadı. Lütfen tekrar dene.",
        type: "Hata",
      });
    }
  };
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={[styles.subheading, { color: theme.textSecondary }]}>
              Bu seri hakkındaki düşünceleriniz nedir?
            </Text>
          </View>
        </View>
